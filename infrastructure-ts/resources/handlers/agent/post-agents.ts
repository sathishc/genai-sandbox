...
class Lambda implements LambdaInterface {
  ...
  public async handler(event: APIGatewayProxyEvent, context: any): Promise<APIGatewayProxyResult> {
    ...
    try {
      const postData: Agent = JSON.parse(event.body!);
      ...
      const channelParams = {
        ...
        Item: {
          ...
          agentAliasId: postData.agentAliasId,
        }
      };

      logger.debug(`Inserting agent details ${JSON.stringify(channelParams)}`);
      await ddb.put(channelParams).promise();

      logger.debug(JSON.stringify(event));
      logger.debug('Post agent executed successfully!');
    }
    catch (e: any) {
